...
class _MienBacScreenState extends State<MienBacScreen> {
  ...
  Future<List<Map<String, dynamic>>> fetchLotteryResults() async {
    final response = await http.get(Uri.parse('https://xskt.com.vn/rss-feed/mien-bac-xsmb.rss'));
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              final results = snapshot.data!.where((result) {
                final pubDate = DateFormat('EEE, dd MMM yyyy HH:mm:ss zzz').parse(result['pubDate']);
                return pubDate.year == selectedDate.year && pubDate.month == selectedDate.month && pubDate.day == selectedDate.day;
              }).toList();

              if (results.isEmpty) {
                return const Center(child: Text('No lottery results for the selected date'));
              }
                  ...
                  child: Column(
                    children: [
                            ...
                            children: [
                              Center(
                                child: Image(
                                  height: 100.h,
                                  width: 300.w,
                                  image: const AssetImage("assets/images/kk-removebg-preview.png"),
                                ),
                              ),
                              Padding(
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      children: [
                                        Row(
                                          ...
                                          children: [
                                            Flexible(
                                              ...
                                              child: TextFormField(
                                                onChanged: (value) {
                                                  enteredValue = value; // Lưu trữ giá trị nhập từ TextFormField
                                                },
                                            ...
                                            Flexible(
                                                ...
                                                child: Container(
                                                  height: 40.h,
                                                  width: 40.w,
                                                  decoration: const BoxDecoration(image: DecorationImage(image: AssetImage("assets/images/qr-scan.png"))),
                                                )),
                                          ],
                                        ),
                                        SizedBox(
                                        ...
                                        DateInputWidget(hintext: "Chọn ngày sổ", controller: dateController, onChanged: (z) {}),
                                        SizedBox(
                                          height: 10.h,
                                        ),
                                        CustomDropdownWidget(
                                          selectedValue: selectedTinh,
                                          items: const ['Tỉnh', 'Tỉnh', 'Tỉnh', 'Tỉnh'],
                                        ...
                                        ElevatedButton(
                                          onPressed: () {
                                            if (enteredValue != null) {
                                              bool found = false;
                                              for (var entry in prizes.entries) {
                                                if (entry.value.contains(enteredValue)) {
                                                  found = true;
                                                  showDialog(
                                                    context: context,
                                                    builder: (context) => AlertDialog(
                                                      title: const Text('Thông báo'),
                                                      content: Text('Giá trị nhập $enteredValue được tìm thấy trong bảng.'),
                                                      actions: [
                                                        TextButton(
                                                          onPressed: () {
                                              ...
                                              if (!found) {
                                                ...
                                                showDialog(
                                                  ...
                                                  builder: (context) => AlertDialog(
                                                    title: const Text('Thông báo'),
                                                    content: Text('Giá trị nhập $enteredValue không được tìm thấy trong bảng.'),
                                                    actions: [
                                                      TextButton(
                                                        onPressed: () {
                                                          Navigator.pop(context);
                                          ...
                                          style: ElevatedButton.styleFrom(
                                            ...
                                            shape: RoundedRectangleBorder(
                                              borderRadius: BorderRadius.circular(5.0.r), // Adjust the border radius
                                            ),
                                          ),
                                          child: const Text("Dò nhanh"), // Text inside the button
                                        ),
                                      ],
                                    ),
                                  ),
                      ...
                      Padding(
                        ...
                        child: DateInputWidget(hintext: "Chọn ngày sổ", controller: dateController, onChanged: (z) {}),
                      ),
                      SizedBox(
                        height: 10.h,
                      ),
                      Table(
                        border: TableBorder.all(color: const Color.fromARGB(158, 158, 158, 158)),
                              ...
                              Wrap(
                                  ...
                                  children: entry.value.map((number) {
                                    ...
                                    if (isFirstRow) {
                                      ...
                                      textStyle = isEven ? styleTitleSmallItem : styleTitleSmallItem;
                                    }
